...
describe('default-theme tests', () => {
  ...
  it.concurrent.each(getAliasTokensTable)('createColorAliasTokens test', async (mode: any, tokens: any) => {
    expect(createColorAliasTokens(mode)).toStrictEqual(mapPipelineToTheme(tokens));
  });
});
